...
class Make
{
    ...
    static function makeModel($name)
    {
        ...
        if (!file_exists($filename)) {
            if (file_exists(self::$tempDirectory . 'Model.ralph')) {
                ...
                chdir(self::$tempDirectory);
                $c = getcwd();
                echo "no temp file in {$c}\n";
            }
    ...
    static function makecontroller($name)
    {
        ...
        if (!file_exists($filename)) {
            ...
            if (file_exists(self::$tempDirectory . 'Controller.ralph')) {
                $handle = fopen($filename, 'w');
                fwrite($handle, '');
                fclose($handle);
                // api_v2/vendor/codx/comic/src/tmp/Controller.ralph
